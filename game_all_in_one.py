...
class ShootingMob(pygame.sprite.Sprite):
    ...
    def shoot(self, all_sprites, bullets, now, image):
        ...
        if now - self.last_shot > self.shoot_delay:
            self.last_shot = now
            ebullet = Bullet(self.rect.left, self.rect.centery, image, True, 10 )
...
if getattr(sys, 'frozen', False):
    ...
    img_dir = path.join(os.path.dirname(sys.executable), 'images')
else:
    # unfrozen
    img_dir = path.join(path.dirname(__file__), 'images')
...
def play_again(max_pkt):
    ...
    if getattr(sys, 'frozen', False):
        ...
        # dir_ = os.path.dirname(sys.executable)
        #img_dir = path.join(os.path.dirname(sys.executable), 'images')
        snd_dir = path.join(os.path.dirname(sys.executable), 'sounds')
    else:
        # unfrozen
        #snd_dir = path.join(path.dirname(__file__), 'sounds')
        snd_dir = path.join(os.path.dirname(sys.executable), 'sounds')
    ...
    def newmob():
        m1 = Mob(basic_mob, WIDTH, HEIGHT, 80, 40)
        m2 = BouncingMob(bouncing_mob, WIDTH, HEIGHT, 80, 40)
        m3 = ShootingMob(shooting_mob, WIDTH, HEIGHT, 80, 80, pygame.time.get_ticks())
    ...
    player = Player(player_img, WIDTH, HEIGHT, 80, 40, 0.5, pygame.time.get_ticks())
    ...
    while running:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_q and player.quantum_bombs > 0:
                    ...
                    for mob in mobs:
                        ...
                        expl = Explosion(mob.rect.center, 'basic', explosion_anim)
        ...
        for hit in hitsPlayer:
            score += hit.score
            random.choice(expl_sounds).play()
            expl = Explosion(hit.rect.center, 'basic', explosion_anim)
            all_sprites.add(expl)
            if len(mobs) < SPAWN:
                newmob()
        ...
        for cols in collide:
            random.choice(expl_sounds).play()
            expl = Explosion(cols.rect.center, 'basic', explosion_anim)
            player.kill()
            all_sprites.add(expl)
            player_lives = False
        ...
        text = "Number of quantum bombs: " + str(player.quantum_bombs)
        draw_text(screen, text, 18, WIDTH / 2, 30)
